package studio.mevera.scofi.animation;

import org.bukkit.ChatColor;
import org.jetbrains.annotations.NotNull;

import java.util.ArrayList;
import java.util.List;

/**
 * Utility class for generating horizontal scrolling (marquee) text frames.
 * <p>
 * Used to create animated scrolling effects for scoreboards and text displays. Handles legacy color codes and
 * ensures consistent width by padding frames with spaces.
 * </p>
 * <p>
 * Usage example:
 * <pre>
 *     Scroller scroller = Scroller.of("Welcome!", 10, 2);
 *     String frame = scroller.next();
 * </pre>
 */
public final class Scroller {

	private int position;
	private final List<String> list;
	private ChatColor color = ChatColor.RESET;
	private final int targetWidth; // Store the target width for padding

	private Scroller(String message, int width, int spaceBetween) {
		message = ChatColor.translateAlternateColorCodes('&', message);
		this.targetWidth = width; // Store original width for padding
		list = new ArrayList<>();
		// Ensure message is at least as wide as the window
		if (message.length() < width) {
			StringBuilder sb = new StringBuilder(message);
			while (sb.length() < width)
				sb.append(" ");
			message = sb.toString();
		}
		// Allow for colours which add 2 to the width
		width -= 2;
		// Invalid width/space size
		if (width < 1)
			width = 1;
		if (spaceBetween < 0)
			spaceBetween = 0;
		// Add substrings with consistent padding
		for (int i = 0; i < message.length() - width; i++) {
			String frame = message.substring(i, i + width);
			// Pad to ensure consistent width
			list.add(padToWidth(frame, width));
		}
		// Add space between repeats
		StringBuilder space = new StringBuilder();
		for (int i = 0; i < spaceBetween; ++i) {
			String frame = message.substring(message.length() - width + (Math.min(i, width))) + space;
			list.add(padToWidth(frame, width));
			if (space.length() < width)
				space.append(" ");
		}
		// Wrap
		for (int i = 0; i < width - spaceBetween; ++i) {
			String frame = message.substring(message.length() - width + spaceBetween + i) + space + message.substring(0, i);
			list.add(padToWidth(frame, width));
		}
		// Join up
		for (int i = 0; i < spaceBetween; i++) {
			if (i > space.length())
				break;
			String frame = space.substring(0, space.length() - i) + message.substring(0, width - (Math.min(spaceBetween, width)) + i);
			list.add(padToWidth(frame, width));
		}
	}
	/**
	 * Pads a string to ensure consistent width
	 */
	private String padToWidth(String str, int width) {
		// Strip existing color codes for length calculation
		String stripped = ChatColor.stripColor(str);
		int actualLength = stripped.length();
		if (actualLength < width) {
			StringBuilder padded = new StringBuilder(str);
			// Add invisible padding (spaces) to maintain width
			for (int i = actualLength; i < width; i++) {
				padded.append(" ");
			}
			return padded.toString();
		}
		return str;
	}

	public static @NotNull Scroller of(String message, int width, int spaceBetween) {
		return new Scroller(message, width, spaceBetween);
	}

	public String next() {
		StringBuilder sb = getNext();
		if (sb.charAt(sb.length() - 1) == ChatColor.COLOR_CHAR) {
			sb.setCharAt(sb.length() - 1, ' ');
		}
		if (sb.charAt(0) == ChatColor.COLOR_CHAR) {
			ChatColor c = ChatColor.getByChar(sb.charAt(1));
			if (c != null) {
				color = c;
				sb = getNext();
				if (sb.charAt(0) != ' ') {
					sb.setCharAt(0, ' ');
				}
			}
		}
		// Ensure consistent width by padding
		String result = color + sb.toString();
		// Add padding to maintain scoreboard width
		StringBuilder paddedResult = new StringBuilder(result);
		while (ChatColor.stripColor(paddedResult.toString()).length() < targetWidth) {
			paddedResult.append(" ");
		}
		return paddedResult.toString();
	}

	private StringBuilder getNext() {
		return new StringBuilder(list.get(position++ % list.size()));
	}

}